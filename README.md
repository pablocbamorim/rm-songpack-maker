...
**Time-agnostic songs on save:** a normal entry that names a biome or biome tag but leaves every time-of-day condition unchecked is expanded into four per-time entries before biome pooling. Its songs are merged into matching DAY/NIGHT/SUNRISE/SUNSET siblings when those siblings have the identical place/other conditions and flags; otherwise a new per-time entry is created. Complex time OR expressions and forceStop*/forceStart* entries are left alone and reported in the Saved dialog. Turn this off in **Settings** ("When saving, expand place-conditioned songs with no time into per-time pools").
