...
- Unsaved-changes protection: the title bar shows "• unsaved changes" the moment anything in the songpack changes, `Ctrl+S` (or File > Save) saves straight back to the current folder without asking for it again, and New Songpack / Load Config… / closing the window all confirm first if there's unsaved work.
...
- **Undo/redo for condition edits.** Unsaved-changes tracking, the Exit/New/Load warnings and Ctrl+S quick-save described above are done; an undo/redo stack for condition edits is still open.
